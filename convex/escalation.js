...
export const checkAndEscalateTasks = internalAction({
    ...
    handler: async (ctx, args) => {
        ...
        const timedOutTasks = await ctx.runQuery(internal.escalation.getTimedOutTasks, {
            timeoutThreshold,
...
export const getTimedOutTasks = internalQuery({
    ...
    handler: async (ctx, args) => {
        const tasks = await ctx.db
            ...
            .collect();
        return tasks.filter((task) => task.dispatched_at &&
...
export const escalateTask = internalMutation({
    ...
    handler: async (ctx, args) => {
        ...
        const assignment = matchResult.assignments.find((a) => a.task_id === args.task_id);
        if (assignment) {
