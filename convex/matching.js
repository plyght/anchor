...
function isAvailableNow(availabilitySchedule) {
    const now = new Date();
    const dayNames = [
        "sunday",
        "monday",
...
function hasRequiredSkills(volunteerSkills, requiredSkills) {
    if (requiredSkills.length === 0)
        return true;
    return requiredSkills.some((skill) => volunteerSkills.includes(skill));
}
function calculateMatchScore(volunteer, task, taskLocation) {
    let score = 0;
    if (volunteer.current_status !== "online" && volunteer.current_status !== "responding")
        return 0;
    if (!isAvailableNow(volunteer.availability_schedule))
        return 0;
    const hasSkills = hasRequiredSkills(volunteer.skills, task.required_skills);
    if (!hasSkills)
        return 0;
    score += 50;
    const matchingSkillsCount = task.required_skills.filter((skill) => volunteer.skills.includes(skill)).length;
...
export const matchTasksToVolunteers = query({
    ...
    handler: async (ctx, args) => {
        ...
        const volunteers = await ctx.db
            .query("volunteers")
            .filter((q) => q.or(q.eq(q.field("current_status"), "online"), q.eq(q.field("current_status"), "responding")))
            .collect();
        ...
        for (const task of tasksByPriority) {
            const availableVolunteers = volunteers.filter((v) => !usedVolunteers.has(v._id));
